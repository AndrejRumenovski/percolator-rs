//! Independent arithmetic oracle promoted from the scientific audit into CI.
//! The historical counterexamples remain in validation/; repaired edge cases
//! are covered separately by scientific_contract.rs.

use percolator_rs::stats::{
    qvalues, qvalues_and_peps, qvalues_into, target_count_at_fdr_into, target_mask_at_fdr_into,
    QValueWorkspace, Tdc,
};
...
fn score_patterns(n: usize) -> Vec<Vec<f64>> {
    let mut output = vec![
        vec![7.0; n],
        (0..n).map(|row| (n - row) as f64).collect(),
        (0..n).map(|row| ((n - row) / 2) as f64).collect(),
        (0..n)
            .map(|row| match row % 5 {
                0 | 1 => 3.0,
                2 => 1.0,
...
#[test]
fn exhaustive_independent_tdc_oracle() {
    ...
    assert_eq!(cases, 40_928);
    assert_eq!(fast_path_checks, 327_424);
}
